...
# disable all warnings in pandas
import warnings
...
def UpSetAltair(
    ...
    if sort_order == "levels":
        dfs = []
        levels = ["level_0", "level_1", "level_2", "level_3"]

        for level in levels:
            # Filter dataframe for each level
            df_level = data[data[level] == 1]

            # Sort values based on conditions
            df_level = df_level.sort_values(
                by=[level, "mesh_terms", "strength"], ascending=[True, True, True]
            )

            # Append sorted dataframe to list
            dfs.append(df_level)
    ...
    vertical_bar = (
        ...
        .encode(
            x=alt.X(
                ...
                sort=x_sort if sort_order != "levels" else None,
                title=None,
            ),
            y=alt.Y(
                "max(count):Q",
