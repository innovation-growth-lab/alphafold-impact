options(scipen = 999)
tables <- "data/05_model_output/authors/nonecr/articles/tables/"
...
generate_tables <- function(results, dep_vars, table_info, subsets, cov_sets, fe_list, treat_vars) { # nolint
  ...
  strength_groups <- c("use_General Use", "use_Methodological Use") # nolint
    ...
    for (depth in depths) {
      for (pdb_group in pdb_groups) {
            ...
            for (cov_set in cov_sets) {
              for (fe in fe_list) {
                for (treat_var in treat_vars) {
                  ...
                  result_name <- paste0(
                    depth, "__", field, "__", strength_group, "__", pdb_group, "__", # nolint
        ...
        if (length(result_names) > 0) {
          ...
          # strength headers
          strength_headers <- paste0(
            rep(
              "\\multicolumn{2}{c}{General} & \\multicolumn{2}{c}{Methodological}", # nolint
              length(field_labels)
            )
          )

          strength_headers <- paste0(
            " & ", paste(strength_headers, collapse = " & "), " \\\\"
          )

          strength_cmidrules <- paste0(
            "\\cmidrule(lr){",
          ...
          strength_cmidrules <- paste0(
            paste(strength_cmidrules, collapse = " ")
          )
          ...
          coefficient_cmidrules <- paste0(
            "\\cmidrule(lr){", seq(2, length(field_labels) * 4 + 1, by = 2), "-", # nolint
          ...
          # Insert strength headers after row 6
          etable_lines <- append(etable_lines, strength_headers, after = 7)
          etable_lines <- append(etable_lines, strength_cmidrules, after = 8)
