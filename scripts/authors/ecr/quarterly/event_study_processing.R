...
ecr_data$quarter <- ecr_data$quarter - min(ecr_data$quarter)
...
for (indicator in indicators) {
  ...
  ecr_data[[first_indicator]][is.infinite(ecr_data[[first_indicator]])] <- NA
}
...
ecr_data <- ecr_data %>%
  select(c(
    ...
    grep("rel_treat_", names(ecr_data), value = TRUE),
    "depth",
    "primary_field",
    "high_pdb"
  ))
    ...
    for (sub_group in sub_groups) {
      ...
      if (sub_group == "CEM") {
        ...
        match_out_af <- matchit(
          ...
          data = ecr_data_cem, method = "cem", k2k = FALSE
        )
